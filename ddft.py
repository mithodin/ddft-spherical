...
class DDFT:
    def __init__(self, analysis: Analysis, dt: float, f_exc: Fexc, rho0: Tuple[np.ndarray, np.ndarray],
                 cutoff: Cutoff = None):
        self._ana = analysis
        self._dt = dt
        self._f_exc = f_exc
    ...
    def __boundary_condition(self, d_rho_s: np.ndarray, d_rho_d: np.ndarray) -> None:
    ...
    def j_exc(self) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def norms(self) -> Tuple[float, float]:
